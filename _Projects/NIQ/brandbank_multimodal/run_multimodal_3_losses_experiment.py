...
from src.model import MultimodalModel3LossesModule
...
def train_and_test_model(config_path):
    ...
    model_module = MultimodalModel3LossesModule(
        model_name=model_name,
        label2id=label2id,
    ...
    df_predictions["text_predictions"] = map_col(
        df_predictions["text_predictions"], id2label_series
    )
    df_predictions["image_predictions"] = map_col(
        df_predictions["image_predictions"], id2label_series
    )
    df_predictions["multimodal_predictions"] = map_col(
        df_predictions["multimodal_predictions"], id2label_series
    )

    # Generate prediction metrics
    df_text_prediction_metrics = calculate_metrics(
        df=df_predictions,
        label_colname="labels",
        prediction_colname="text_predictions",
    )
    df_image_prediction_metrics = calculate_metrics(
        df=df_predictions,
        label_colname="labels",
        prediction_colname="image_predictions",
    )
    df_multimodal_prediction_metrics = calculate_metrics(
        df=df_predictions,
        label_colname="labels",
        prediction_colname="multimodal_predictions",
    )

    df_top_10_confusions_text = top_10_confusions(
        df=df_predictions,
        label_colname="labels",
        prediction_colname="text_predictions",
    )
    df_top_10_confusions_image = top_10_confusions(
        df=df_predictions,
        label_colname="labels",
        prediction_colname="image_predictions",
    )
    df_top_10_confusions_multimodal = top_10_confusions(
        df=df_predictions,
        label_colname="labels",
        prediction_colname="multimodal_predictions",
    )
    ...
    # Text
    df_text_prediction_metrics.to_csv(
        save_dir / "text_predictions_metrics.csv", index=False
    )
    df_top_10_confusions_text.to_csv(
        save_dir / "text_predictions_top_10_confusions.csv", index=False
    )

    # Image
    df_image_prediction_metrics.to_csv(
        save_dir / "image_predictions_metrics.csv", index=False
    )
    df_top_10_confusions_image.to_csv(
        save_dir / "image_predictions_top_10_confusions.csv", index=False
    )

    # Multimodal
    df_multimodal_prediction_metrics.to_csv(
        save_dir / "multimodal_predictions_metrics.csv", index=False
    )
    df_top_10_confusions_multimodal.to_csv(
        save_dir / "multimodal_predictions_top_10_confusions.csv", index=False
    )
